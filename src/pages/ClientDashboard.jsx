    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 animate-fadeIn">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2">
            <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-full bg-white/10 text-brand-200 border border-white/15 text-xs font-semibold backdrop-blur-md">
              ...
              <span>{t('dashboard.accountWorkspace', 'Account Workspace')}</span>
              <span className="text-white/40">•</span>
              <span className="font-mono text-amber-300 font-bold">{clientId}</span>
            </div>
            <h1 className="text-2xl sm:text-3xl font-black tracking-tight text-white">
              {t('dashboard.welcomeBack', 'Welcome back, {name}!', { name: currentUser?.name || 'Enterprise Client' })}
            </h1>
            <p className="text-sm text-slate-300 max-w-2xl">
              {t('dashboard.subtitle', 'Track your industrial clearances, sequence dependencies, and statutory approvals in one place.')}
            </p>
          </div>
          ...
          <div className="flex items-center space-x-3 shrink-0">
            ...
            >
              ...
              <span>{t('dashboard.startNewApplication', '+ Start New Application')}</span>
            </button>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="p-4 rounded-2xl bg-white border border-slate-200 shadow-2xs">
          <div className="text-xs font-bold text-slate-500 uppercase">{t('dashboard.totalApplications', 'Total Applications')}</div>
          <div className="text-2xl font-black text-slate-900 mt-1">{applications.length}</div>
        </div>
        <div className="p-4 rounded-2xl bg-white border border-slate-200 shadow-2xs">
          <div className="text-xs font-bold text-amber-700 uppercase">{t('dashboard.inProgress', 'In Progress')}</div>
          <div className="text-2xl font-black text-amber-600 mt-1">{activeApplications.length}</div>
        </div>
        <div className="p-4 rounded-2xl bg-white border border-slate-200 shadow-2xs">
          <div className="text-xs font-bold text-emerald-700 uppercase">{t('dashboard.completed', 'Completed')}</div>
          <div className="text-2xl font-black text-emerald-600 mt-1">
            {applications.filter(a => a.status === 'Completed').length}
          </div>
        </div>
        <div className="p-4 rounded-2xl bg-white border border-slate-200 shadow-2xs">
          <div className="text-xs font-bold text-blue-700 uppercase">{t('dashboard.submitted', 'Submitted')}</div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <h2 className="text-lg font-bold text-slate-900">{t('dashboard.activeApplications', 'Active In-Progress Applications')}</h2>
        ...
        {loading ? (
          <div className="p-12 text-center bg-white rounded-3xl border border-slate-200">
            ...
            <p className="text-xs font-medium text-slate-500">{t('common.loading', 'Loading...')}</p>
          </div>
                ...
                >
                  <div className="space-y-3">
                      ...
                      <div>
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="px-2 py-0.5 text-[10px] font-bold rounded-md bg-amber-100 text-amber-800 border border-amber-200">
                            {t('common.inProgress', app.status)}
                          </span>
                        </div>
                    ...
                    <div className="space-y-1.5 pt-2">
                      <div className="flex justify-between text-xs font-semibold">
                        <span className="text-slate-600">{t('dashboard.overallProgress', 'Overall Progress')}</span>
                      ...
                      <div className="flex items-center justify-between text-[11px] text-slate-500 pt-1">
                        <span>{t('dashboard.clearancesTracked', '{count} Clearances Tracked', { count: approvalsCount })}</span>
                        <span>{t('dashboard.documentsApproved', '{approved}/{total} Documents Approved', { approved: approvedDocsCount, total: docsCount })}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-5 pt-4 border-t border-slate-100 flex items-center justify-between">
                    <span className="text-[11px] text-slate-400">
                      {new Date(app.updatedAt || app.createdAt || Date.now()).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}
                    </span>
                    <button
                    ...
                    >
                      <span>{t('dashboard.continueApplication', 'Continue Application')}</span>
          ...
          <div className="bg-white rounded-3xl border-2 border-dashed border-slate-200 p-8 sm:p-12 text-center space-y-4">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-800">{t('dashboard.noActiveApplications', 'No active applications in progress')}</h3>
              <p className="text-xs text-slate-500 max-w-sm mx-auto mt-1">
                {t('dashboard.noActiveDesc', 'You do not have any unfinished applications right now. Create a new one to discover required approvals and sequence.')}
              </p>
            </div>
            <button
              onClick={() => onStartNewApplication ? onStartNewApplication() : onNavigate('/application/new')}
            ...
            >
              <Plus className="w-4 h-4" />
              <span>{t('dashboard.startFirstApp', 'Start Your First Application')}</span>
            </button>
          </div>
        )}
      </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <h2 className="text-lg font-bold text-slate-900">{t('dashboard.submittedCompletedDossiers', 'Submitted & Completed Dossiers')}</h2>
            ...
            {completedOrSubmitted.map((app) => {
              ...
              return (
                ...
                >
                  <div className="space-y-1">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className={`px-2 py-0.5 text-[10px] font-bold rounded-md ${
                        ...
                        {app.status === 'Completed' ? t('common.completed', 'Completed') : t('dashboard.submitted', 'Submitted')}
                      </span>
                    </div>
                    <div className="text-sm font-bold text-slate-800">{t('industries.' + app.industry, app.industry)} ({app.district}, {t('states.' + app.state, app.state)})</div>
                    {app.certificateNumber && (
                      <div className="text-[11px] font-mono text-emerald-700">
                        {t('dashboard.licenseNo', 'License No')}: {app.certificateNumber}
                      </div>
                    )}
                  </div>
                  <button
                  ...
                  >
                    {t('dashboard.viewDossier', 'View Dossier →')}
                  </button>
                </div>
              );
            })}
      ...
      <div className="bg-white rounded-3xl border border-slate-200 p-6 space-y-4 shadow-xs">
        <div className="flex items-center space-x-2">
          ...
          <h2 className="text-base font-bold text-slate-900">{t('dashboard.recentActivity', 'Recent Account Activity')}</h2>
        </div>
        ...
        {allTimelineEvents.length > 0 ? (
                      ...
                      <div className="flex min-w-0 flex-1 justify-between space-x-4 pt-1.5">
                        <div>
                          <p className="text-xs font-bold text-slate-800">
                            {event.event} <span className="text-slate-400 font-normal">{t('common.for', 'for')}</span> <span className="font-mono text-brand-700 font-semibold">{event.appId}</span>
                          </p>
                          {event.remarks && (
          ...
          <p className="text-xs text-slate-500">{t('dashboard.noActivity', 'No activity recorded yet.')}</p>
        )}
      </div>
